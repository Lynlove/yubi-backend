...
//        chart.setGenResult(genResult);
        ...
        CompletableFuture.runAsync(() -> {
            ...
            updateChart.setId(finalChart.getId());
            // 把任务状态改为执行中
            updateChart.setStatus("running");
            boolean b = chartService.updateById(updateChart);
            // 如果提交失败(一般情况下,更新失败可能意味着你的数据库出问题了)
            if (!b) {
                handleChartUpdateError(finalChart.getId(), "更新图表执行中状态失败");
                return;
            }
            ...
            String[] splits = result.split("【【【【【");
            if (splits.length < 3) {
                handleChartUpdateError(finalChart.getId(), "AI 生成错误");
                return;
            }
            String genChart = splits[1].trim();
            genChart = genChart.replace("'", "\"");
            ...
            updateChartResult.setId(finalChart.getId());
            updateChartResult.setGenChart(genChart);
            updateChartResult.setGenResult(genResult);
            updateChartResult.setStatus("succeed");
            boolean updateResult = chartService.updateById(updateChartResult);
            if (!updateResult) {
                handleChartUpdateError(finalChart.getId(), "更新图表成功状态失败");
            }
        }, threadPoolExecutor);
        BiResponse biResponse = new BiResponse();
//        biResponse.setGenChart(genChart);
